...
const ChangeAvatar = ({ handleExportAvatar, setImage }) => {
  ...
  const handleAvatar = (e) => {
    ...
    fetch(urlPostAvatarEmployee, {
      ...
      .then((data) => {
        ...
        const imgUrl = URL.createObjectURL(fileAvatar); // lấy đường dẫn tạm thời đến file ảnh đã được tải lên
        setImage(imgUrl);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    fetch(
      `${process.env.REACT_APP_API_ENDPOINT}api/v2/employee/avatar/${fetchAvaApis}`,
      {
        headers: {
          Authorization: `Bearer ${tokenTaken}`,
        },
